...
def get_parser():
    ...
    # parser.add_argument('--config', type=str, default='config/LoveDA/LoveDA_split0_resnet50_MGANet.yaml',
    #                      help='config file')  # coco/coco_split0_resnet50.yaml
    parser.add_argument('--config', type=str, default='config/iSAID/iSAID_split0_resnet50_MGANet.yaml',
                        help='config file')  # coco/coco_split0_resnet50.yaml
...
def main():
    ...
    global best_miou_set, best_FBiou_set, best_piou_set, best_epoch_set
    best_miou = 0.
    best_FBiou = 0.
    best_piou = 0.
    best_epoch = 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if ((epoch + 1) % args.save_freq == 0) and (epoch > 0) and main_process():
...
def train(train_loader, val_loader, model, optimizer, epoch, proto_dict=None,bp_proto_dict=None):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        #supp_loss_meter.update(supp_loss.item(), n)
            ...
            if mIoU > best_miou:
                ...
                if main_process():
                    ...
                    torch.save(
                        ...
                        filename)
                    #torch.save(proto_dict, args.snapshot_path + "/train_epoch_" + str(epoch - 0.5) + '{:.4f}'.format(best_miou) + "proto.pth")
...
def validate(val_loader, model, proto_dict, current_iter):
    ...
    if args.manual_seed is not None and args.fix_random_seed_val:
        #setup_seed(args.manual_seed, args.seed_deterministic)
        setup_seed(args.test_seed, args.seed_deterministic)
