...
class PrototypeContrastLoss(nn.Module, ABC):
    def __init__(self):
        ...
        # self.afha1 = nn.Parameter(torch.FloatTensor([0.5]))
        # self.afha2 = nn.Parameter(torch.FloatTensor([0.5]))
    ...
    def _class_construct(self, pros, labels_):  # 0,0,0,1,2,3,4
        ...
        pros = torch.cat(pros,dim=1) # bs,k,c,1,1 这里形状是对的
        ...
        for i in range(len(unique_labels)): #0
            ...
            pro_dict[unique_labels[i].item()] = pros[index].contiguous().view(-1, c)  # {0:Tensor(1,256),1:,,,,2:,,,}
    ...
    def forward(self, s_fp_list, s_bp_list, classes, proto_dict,bp_proto_dict):
        ...
        for key in class_dict.keys():
            if key not in proto_dict:
                ...
                orignal_value = proto_dict[key]# * self.afha1 #1,2048
                proto_dict[key] = torch.mean(torch.cat([orignal_value,class_dict[key]],dim=0),dim=0,keepdim=True).detach()
                #proto_dict[key] = 0.
                # proto_dict[key] = (torch.mean(class_dict[key], dim = 0, keepdim=True) *  + \
                #                                             (1 - self.afha1) + orignal_value).detach()
        for key in class_bp_dict.keys():
            if key not in bp_proto_dict:
                ...
                orignal_value = bp_proto_dict[key] #* self.afha2
                bp_proto_dict[key] = torch.mean(torch.cat([orignal_value,class_bp_dict[key]],dim=0),dim=0,keepdim=True).detach()
                # bp_proto_dict[key] = (torch.mean(class_bp_dict[key], dim = 0, keepdim=True) *  + \
                #                                             (1 - self.afha2) + orignal_value).detach()
